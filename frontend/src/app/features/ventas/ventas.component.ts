...
export class VentasComponent implements OnInit {
  ...
  exportarCSV() {
    if (this.ventas.length === 0) { this.toast.info('No hay ventas para exportar'); return; }
    ...
    const csv = [cabecera, ...filas].map(f => f.map(c => `"${c}"`).join(';')).join('\n');
  ...
  imprimirBoleta() {
    ...
    const html = `<!DOCTYPE html><html lang="es"><head>
      <meta charset="UTF-8"><title>Boleta ${v.numero_boleta}</title>
      <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .tline{display:flex;justify-content:space-between;padding:3px;
      ...
      <div class="meta">
        ...
        <div class="boleta-d">${new Date(v.creado_en).toLocaleString('es-CL')} &nbsp;·&nbsp;
          ${this.etiquetaMetodo(v.metodo_pago)}</div>
      </div>
      <table><thead><tr>
      ...
      <div class="totals">
        ...
        ${v.descuento > 0
          ? `<div class="tline" style="color:#059669">
               <span>Descuento</span><span>−${this.formatearMonto(v.descuento)}</span></div>` : ''}
